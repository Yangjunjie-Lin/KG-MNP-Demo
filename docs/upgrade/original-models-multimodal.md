...
首次推送后的干净 CI 发现本地 Ruff 旧缓存隐藏的兼容命名空间导入分类问题。后续显式声明 first-party 命名空间、整理十处导入，并强制验收/CI 使用无缓存 lint，同时更新对应的工作流命令断言。应用和前端源码、模型逻辑、依赖和业务规则未改；十个导入整理文件的导入集合与其余 AST 一致，受影响的 59 项测试通过。上述实测摘要保留为原测试快照，不将格式/验收脚本修正伪装成重跑模型。
