...
def read_ready(server, timeout: float) -> dict:
    ...
    try:
        ...
        if not workspace.is_relative_to((ROOT / "runtime").resolve()) or credential.is_symlink() or credential.resolve(strict=True) != workspace / "browser-test-credential.json":
    ...
    try:
        with (directory / "server.log").open("w", encoding="utf-8") as log:
            ...
            try:
                ...
                ready = read_ready(server, args.startup_timeout)
