...
def test_request_too_large(client, monkeypatch):
    c, _, _ = client
    monkeypatch.setenv("KG_MNP_MAX_REQUEST_BYTES", "200")
    # recreate app to pick up env? middleware reads at init — set on existing via attribute
    for m in c.app.user_middleware:
        pass
    # Directly patch middleware instance
    for mw in c.app.middleware_stack.__dict__.get("app", []) if False else []:
        pass
    from kg_mnp_demo.api.middleware.request_size import RequestSizeLimitMiddleware

    # Rebuild client with small limit by constructing middleware manually
    from kg_mnp_demo.api.dependencies import AppState
    from kg_mnp_demo.storage import AssessmentRepository, ArtifactRepository, Database
    from pathlib import Path
    import tempfile

    # Use TestClient against fresh app then replace middleware max
    app = c.app
    # Walk middleware to find RequestSizeLimitMiddleware
    found = False

    def _find(obj):
        nonlocal found
        if isinstance(obj, RequestSizeLimitMiddleware):
            obj.max_bytes = 200
            found = True
            return
        if hasattr(obj, "app"):
    ...
    _find(app.middleware_stack)
    if not found:
        pytest.skip("middleware instance not reachable")
    huge = {"payload": {**_case(), "pad": "x" * 5000}, "persist": False}
    # pad will fail pydantic extra forbid — use raw body
    resp = c.post(
        "/api/v1/assessments",
        content=b'{"payload":' + (b"x" * 5000) + b"}",
        headers={"content-type": "application/json", "content-length": "5015"},
    )
