...
def test_system_observation_infers_evidence_record():
    ...
    apply_owlrl(g)
    assert (DATA["Ev-01-ID"], RDF.type, MNP.SystemObservation) in g
    assert (DATA["Ev-01-ID"], RDF.type, MNP.EvidenceRecord) in g
...
def test_inference_is_deterministic():
    ...
    assert (DATA["Ev-02-ID"], RDF.type, MNP.EvidenceRecord) in g1
    assert (DATA["Ev-02-ID"], RDF.type, MNP.EvidenceRecord) in g2
