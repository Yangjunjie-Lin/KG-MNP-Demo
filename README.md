...
本分支正在追加[TXT / Word / Excel 混合资料建模](docs/user-guide/mixed-source-modeling.md)：明确表格选择、业务主键与别名、原文模板/片段标注、类型转换和漏映射阻断。旧 RC1 验收不覆盖这些新修改；它们需要新的固定修订验收。这里的去重是证据支持的明确身份规则，不是对任意文档的全自动语义理解保证。
