...
def load_configuration(workspace_root: Path | str = ".") -> ServiceConfiguration:
    ...
    configuration = replace(configuration, reasoner_jar=get_setting("REASONER_JAR"),
                            ontology_trace_enabled=get_setting("ONTOLOGY_TRACE_ENABLED", "false").lower() == "true",
                            modeling_audit_content_enabled=get_setting("MODELING_AUDIT_CONTENT_ENABLED", "false").lower() == "true",
                            ontology_trace_data_policy=get_setting("ONTOLOGY_TRACE_DATA_POLICY", "AUTHORIZED_PUBLIC_CONTENT_ONLY"),
