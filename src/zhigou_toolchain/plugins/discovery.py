...
def discover_builtin_plugins() -> tuple[PluginDescriptor, ...]:
    ...
    manifest_root = resources.files("zhigou_toolchain.plugins.builtin").joinpath("manifests")
    package_root = Path(resources.files("zhigou_toolchain.plugins"))
...
def discover_external_plugins() -> tuple[PluginDescriptor, ...]:
    ...
    for entry_point in sorted(_entry_points(), key=lambda item: (item.name, item.value)):
        ...
        if distribution is None:
            continue
        if distribution.metadata["Name"].casefold().replace("_", "-") in {"kg-mnp-toolchain", "zhigou-toolchain"}:
            continue
        try:
