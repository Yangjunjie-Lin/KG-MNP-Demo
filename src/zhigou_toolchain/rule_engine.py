...
from zhigou_toolchain.loader import rules_path
from zhigou_toolchain.namespaces import DATA

ASSESSMENT_TIME = datetime(2026, 7, 1, 0, 0, 0, tzinfo=UTC)
    ...
    for rule_id, versions in by_id.items():
        ...
        for i in range(len(intervals)):
            ...
            for j in range(i + 1, len(intervals)):
                ...
                end1 = e1 or datetime.max.replace(tzinfo=UTC)
                end2 = e2 or datetime.max.replace(tzinfo=UTC)
...
def _is_applicable(rule: dict[str, Any], assessment_time: datetime) -> bool:
    ...
    if assessment_time < start:
        return False
    return not (end is not None and assessment_time > end)
...
class EvidenceView:
    ...
    def is_usable(self, as_of: datetime = ASSESSMENT_TIME) -> bool:
        if self.status != "VALID":
            return False
        return not (self.valid_until and self.valid_until < as_of)
    ...
    *,
    ...
    for rule in rules:
        ...
        if not usable:
            ...
            continue
        chosen = min(usable, key=lambda e: e.iri)
