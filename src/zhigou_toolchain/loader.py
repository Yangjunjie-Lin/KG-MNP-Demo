...
from zhigou_toolchain.namespaces import CASE_FILES
from zhigou_toolchain.paths import domain_pack_path, repository_root

ROOT = repository_root()
MNP_PACK = domain_pack_path("mnp")
...
SHAPE_PROFILES = {
    ...
    "eligibility": [
        "shapes/foundation-instance-shapes.ttl",
        "fixtures/eligibility-use-case/shapes/eligibility-instance-shapes.ttl",
    ],
    "ontology_schema": [
...
def ontology_modules_config_path() -> Path:
    return MNP_PACK / "ontology" / "modules.yaml"
...
def ontology_module_entries(*, include_alignments: bool = False) -> list[dict[str, Any]]:
    ...
    for entry in modules:
        ...
        if is_runtime or include_alignments and (is_optional or code == "ALIGNMENTS"):
...
def ontology_paths(*, include_alignments: bool = False) -> list[Path]:
    ...
    for entry in ontology_module_entries(include_alignments=include_alignments):
        path = MNP_PACK / "ontology" / entry["file"]
        if not path.is_file():
...
def shapes_path() -> Path:
    ...
    return MNP_PACK / "fixtures" / "eligibility-use-case" / "shapes" / "eligibility-instance-shapes.ttl"
...
def shape_paths(profile: str = "eligibility") -> list[Path]:
    ...
    paths = [MNP_PACK / rel for rel in SHAPE_PROFILES[profile]]
    for path in paths:
        if not path.is_file():
...
def reference_paths() -> list[Path]:
    return [
        MNP_PACK / "fixtures" / "data" / "regulations.ttl",
        MNP_PACK / "fixtures" / "data" / "reference_systems.ttl",
    ]
...
def case_path(case_id: str) -> Path:
    ...
    return MNP_PACK / "fixtures" / "data" / CASE_FILES[case_id]
...
def rules_path() -> Path:
    return MNP_PACK / "rules" / "eligibility_rules.yaml"


def mappings_path() -> Path:
    return MNP_PACK / "mappings" / "tmf_to_mnp.yaml"


def source_manifest_path() -> Path:
    return MNP_PACK / "terminology" / "source_manifest.yaml"


def query_path(name: str) -> Path:
    return MNP_PACK / "queries" / name
