...
def build_cq_test_plan(
    ...
    *,
    ...
    for source in tests:
        ...
        profiles = [a.get("string_values", []) for a in row["assertions"] if a["assertion_type"] == "RESULT_SEMANTIC_HASH"]
        if any(p not in ([], ["SET_V1"], ["ORDERED_V1"]) for p in profiles):
            raise ValueError("unknown result hash comparison profile")
        if ["ORDERED_V1"] in profiles:
            from rdflib.plugins.sparql.parser import parseQuery
            if query_type != "SELECT" or "orderby" not in parseQuery(query)[1]:
                raise ValueError("ORDERED comparison requires top-level ORDER BY")
        ...
        if operation == "ASK":
            ...
            for result in answer:
                ...
                if len(rows) > max_results:
                    ...
                    return
            value = {"variables": variables, "rows": rows if ordered else sorted(rows, key=semantic_hash)}
        else:
...
def _execute(
    ...
    process = context.Process(
        ...
        args=(nquads, query, operation, max_results, output, ordered),
    )
    process.start()
    try:
...
def _assertions(operation: str, result: dict[str, Any], assertions: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int, str]:
    ...
    for assertion in assertions:
        ...
        if kind == "BOOLEAN_EQUALS" and operation == "ASK":
            ...
            profile = assertion.get("string_values", [])
            if profile == ["SET_V1"] and operation == "SELECT":
                unique = {semantic_hash(row): row for row in result["rows"]}
                normalized = {"variables": result["variables"], "rows": [unique[k] for k in sorted(unique)]}
                passed = semantic_hash(normalized) == assertion["semantic_hash"]
            elif profile in ([], ["ORDERED_V1"]):
        ...
        if hashlib.sha256(query_bytes).hexdigest() != test["query_sha256"]:
            ...
            try:
                ...
                execution, value = _execute(
                    ...
                    any(a["assertion_type"] == "RESULT_SEMANTIC_HASH" and a.get("string_values") == ["ORDERED_V1"] for a in test["assertions"]),
                )
        if execution == "OK":
