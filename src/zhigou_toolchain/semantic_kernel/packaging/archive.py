    ...
    for directory, directories, names in os.walk(root, followlinks=False):
        ...
        for name in names:
            ...
            with path.open("rb") as stream:
                # Bound allocation to the observed file, not the whole 1 GiB
                # archive budget. An extra byte detects concurrent growth.
                data = stream.read(size + 1)
            if len(data) != size:
                raise PackageError("package entry changed while capturing", code="PACKAGE_ARCHIVE_INVALID")
            total += len(data)
