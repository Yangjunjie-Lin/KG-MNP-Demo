...
from .agents import AGENT_PACKAGES, OWNERS
...
def project_flow(project_id: str, results: list[dict], jobs: list[dict]) -> dict:
    ...
    return {"schema_version": "1.0.0", "session_id": session_id, "authority": "OBSERVATION_ONLY",
            "agent_roles": OWNERS, "agent_packages": AGENT_PACKAGES, "agent_runs": [r["result"]["agent_execution"] for r in results if "agent_execution" in r["result"]],
