...
ONTOLOGY_BASELINE_PATH = MNP_PACK / "ontology" / "ontology-baseline-1.0.0.json"
MAPPING_RULES_PATH = MNP_PACK / "mappings" / "modeling-rules-1.0.0.yaml"
TERMINOLOGY_PROFILE_PATH = MNP_PACK / "terminology" / "terminology-profile-1.0.0.yaml"
...
def _module_record(
    ...
    *,
    ...
    source = domain_pack_path("mnp", repository=root) / "ontology" / str(entry["file"])
...
def build_ontology_baseline_manifest(
    ...
    module_config_path = (
        domain_pack_path("mnp", repository=root) / "ontology" / "modules.yaml"
    )
    ...
    if not isinstance(root_config, Mapping) or not isinstance(modules, list):
        raise DependencyError("invalid MNP ontology modules.yaml structure")
...
def verify_ontology_baseline_manifest(
    ...
    *,
    ...
    path = (
        manifest_path
        or domain_pack_path("mnp", repository=root)
        / "ontology"
...
def load_modeling_dependencies(
    *,
    ...
    pack_dir = domain_pack_path("mnp", repository=root)
    baseline = load_ontology_baseline(
        pack_dir / "ontology" / ONTOLOGY_BASELINE_PATH.name
    )
    mapping_rules = load_mapping_rules(
        pack_dir / "mappings" / MAPPING_RULES_PATH.name
    )
    terminology = load_terminology_profile(
        pack_dir / "terminology" / TERMINOLOGY_PROFILE_PATH.name
    )
