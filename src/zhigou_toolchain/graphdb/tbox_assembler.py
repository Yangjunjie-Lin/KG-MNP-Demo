...
def assemble_runtime_tbox(*, root: Path = ROOT, baseline: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    root_config = _safe_module_path(
        root, "domain_packs/mnp/ontology/kg-mnp.ttl"
    )
    root_graph = _parse(root_config)
    root_iri = root_tbox_graph_iri(str(baseline["release_source_hash"]))
    modules.append({"code": "root", "path": "domain_packs/mnp/ontology/kg-mnp.ttl", "source_hash": baseline["release_source_hash"], "graph_iri": root_iri, "graph": root_graph, "ontology_iri": baseline["root_ontology_iri"], "version_iri": baseline["root_version_iri"]})
