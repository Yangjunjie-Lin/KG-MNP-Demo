...
from zhigou_toolchain.contracts.document_io import parse_json_bytes
from zhigou_toolchain.contracts.errors import DocumentError
...
from zhigou_toolchain.services.requests import (
    ChangeEvaluationRequest,
    CompileBuildRequest,
    CompileExactPlanRequest,
    CompilePlanRequest,
    ConsumerRequest,
    ...
    ModelingProfileRequest,
    ModelingScopeDraftRequest,
    ObjectRequest,
    ObjectTraceRequest,
    PackageRequest,
...
def create_app(service: ApplicationService) -> FastAPI:
    app = FastAPI(title="ZhiGou Toolchain API", version=__version__, openapi_version="3.1.0")
    ...
    def project_state(project_id: str, authorization: str | None = Header(default=None)):
        ...
        from zhigou_toolchain.services.projects import (
            load_catalog,
        ...
        from zhigou_toolchain.services.execution import verify_module_generation
        verify_module_generation(service, project_handle)
        from zhigou_toolchain.lifecycle.registry.head import read_head
        ...
        if not principal.can("task:read"):
            results = [row for row in results if not row["operation"].startswith(("task.", "evolution.", "business."))]
        jobs = []
        ...
        from zhigou_toolchain.modeling.five_stage.flow import project_flow
        from zhigou_toolchain.services.modeling_sessions import (
            read as read_modeling_session,
        )
        results = sorted(results, key=lambda row: row["revision"])
        return {"project": opened, "results": results, "jobs": jobs,
                "modeling_session": read_modeling_session(project_handle.root),
                "modeling_flow": project_flow(project_id, results, jobs),
    ...
    @app.post("/api/v1/projects/{project_id}/packages/{package_id}/exports", operation_id="preparePackageDownload", status_code=202)
    def prepare_package_download(project_id: str, package_id: str, payload: EmptyRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("package.export", project_id, authorization, {"package_id": package_id}, idempotency_key)

    @app.get("/api/v1/projects/{project_id}/exports/{job_id}/archive", operation_id="downloadExportSnapshot")
    def download_export_snapshot(project_id: str, job_id: str, authorization: str | None = Header(default=None)):
    ...
    def compile_exact_plan(project_id: str, payload: CompileExactPlanRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("compile.plan.exact", project_id, authorization, payload.model_dump(), idempotency_key)

    @app.get("/api/v1/projects/{project_id}/modeling/tutorial", operation_id="modelingTutorial")
    def modeling_tutorial(project_id: str, authorization: str | None = Header(default=None)):
        return resource("modeling.tutorial", project_id, authorization)

    @app.post("/api/v1/projects/{project_id}/modeling/tutorial/seed", operation_id="seedModelingTutorial", status_code=202)
    def modeling_seed(project_id: str, payload: EmptyRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("modeling.tutorial.seed", project_id, authorization, payload.model_dump(), idempotency_key)

    @app.get("/api/v1/projects/{project_id}/modeling/tutorial/files/{name:path}", operation_id="downloadTutorialFile")
    def modeling_tutorial_file(project_id: str, name: str, authorization: str | None = Header(default=None)):
        service.execute(OperationRequest("modeling.methods", project_id), service.authenticate(authorization or ""))
        from zhigou_toolchain.modeling.five_stage.registry import tutorial_files
        content = tutorial_files().get(name)
        if content is None:
            raise ServiceBoundaryError("ARTIFACT_NOT_FOUND", "unknown tutorial reference", status_code=404)
        return Response(content, media_type="application/octet-stream", headers={"Content-Disposition": "attachment"})

    @app.get("/api/v1/projects/{project_id}/modeling/artifacts/{artifact_id}", operation_id="downloadModelingArtifact")
    def modeling_artifact(project_id: str, artifact_id: str, authorization: str | None = Header(default=None)):
        from zhigou_toolchain.contracts.canonical import (
            canonical_json_bytes,
            semantic_hash,
        )
        state = project_state(project_id, authorization)
        found = next((a for a in state["modeling_flow"]["artifacts"] if a["ref"]["artifact_id"] == artifact_id), None)
        if found is None:
            raise ServiceBoundaryError("ARTIFACT_NOT_FOUND", "unknown project artifact", status_code=404)
        if semantic_hash(found["content"]) != found["ref"]["content_hash"]:
            raise ServiceBoundaryError("ARTIFACT_INTEGRITY_FAILED", "artifact hash mismatch", status_code=409)
        return Response(canonical_json_bytes(found["content"]), media_type="application/json",
                        headers={"Content-Disposition": "attachment"})

    @app.post("/api/v1/projects/{project_id}/modeling/profiles", operation_id="profileModelingInput", status_code=202)
    def modeling_profile(project_id: str, payload: ModelingProfileRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("modeling.profile", project_id, authorization, payload.model_dump(), idempotency_key)

    @app.post("/api/v1/projects/{project_id}/modeling/semantic-checks", operation_id="checkModelingSemantics", status_code=202)
    def modeling_semantic_check(project_id: str, payload: SemanticCheckRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("modeling.semantic.check", project_id, authorization, payload.model_dump(), idempotency_key)

    @app.post("/api/v1/projects/{project_id}/modeling/scope-drafts", operation_id="draftModelingScope", status_code=202)
    def modeling_scope_draft(project_id: str, payload: ModelingScopeDraftRequest, authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):
        return resource("modeling.scope.draft", project_id, authorization, payload.model_dump(), idempotency_key)
    ...
    def download_package(project_id: str, package_id: str, authorization: str | None = Header(default=None)):
        principal = service.authenticate(authorization or "")
        from zhigou_toolchain.services.compilation import read_archive
    ...
    @app.post("/api/v1/projects/{project_id}/toolchain/{operation}", operation_id="runToolchainModule", status_code=202)
    def toolchain_module(project_id: str, operation: str, payload: dict[str, Any] = Body(default_factory=dict), authorization: str | None = Header(default=None), idempotency_key: str | None = Header(default=None)):  # noqa: B008
        from zhigou_toolchain.services.business import OPERATIONS as business_operations
        from zhigou_toolchain.services.modeling_sessions import (
            OPERATIONS as session_operations,
        )
        if operation not in business_operations | session_operations | {"module.evaluate", "source.sample.load"}:
            raise ServiceBoundaryError("OPERATION_NOT_FOUND", "Unknown toolchain module operation", status_code=404)
        return resource(operation, project_id, authorization, payload, idempotency_key)
