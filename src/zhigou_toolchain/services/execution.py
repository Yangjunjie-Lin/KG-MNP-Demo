...
def verify_module_generation(service, project, *, observed_digest=None):
    """New session/action authorities are valid only in a server-published generation."""
    root = Path(project.root)
    if not any((root / "registry" / name).exists() for name in ("modeling-session.json", "business-state.json")):
        return
    receipts = [r for r in load_catalog(service.root).get("commits", {}).values()
                if r["context"]["project_id"] == project.project_id and r["authority_revision"] == project.authority_revision
                and Path(r["root"]) == root]
    if len(receipts) != 1:
        raise ServiceBoundaryError("MODULE_AUTHORITY_UNVERIFIED", "Session/action state has no server commit provenance", status_code=409)
    receipt = receipts[0]
    digest = observed_digest or tree_digest(root)
    if receipt["output_tree_digest"] != digest or receipt["receipt_digest"] != semantic_hash({k: v for k, v in receipt.items() if k != "receipt_digest"}):
        raise ServiceBoundaryError("MODULE_AUTHORITY_TAMPERED", "Session/action generation no longer matches its server receipt", status_code=409)
...
def execute_fenced(service, job, request, principal, action):
    ...
    verify_module_generation(service, project, observed_digest=input_digest)
    ...
    try:
        ...
        from .modeling_sessions import after, before
