...
from zhigou_toolchain.lifecycle.store import list_records
...
class ApplicationService:
    ...
    def runtime_check(self) -> dict[str, Any]:
        ...
        from zhigou_toolchain.contracts.catalog import verify_catalog_lock
        ...
        from zhigou_toolchain.semantic_kernel.snapshot import build_compiler_snapshot
    ...
    def execute_job(self, job, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        from .business import OPERATIONS as BUSINESS_OPERATIONS
    ...
    def _execute_inline(self, request: OperationRequest, principal: PrincipalReference, operation: OperationDefinition) -> dict[str, Any]:
        ...
        if name == "business.inspect":
            ...
            verify_module_generation(self, project)
            return execute_business(self, project, request, principal)
        if name == "project.validate":
            return inspect_project(project, packs_root).public_dict()
        if name in {"modeling.methods", "modeling.tutorial"}:
            from zhigou_toolchain.modeling.five_stage.registry import registry, tutorial
            return registry() if name == "modeling.methods" else tutorial()
        ...
        if name == "environment.inspect":
            from zhigou_toolchain.lifecycle.environment import _pointer
            from zhigou_toolchain.lifecycle.errors import LifecycleError
            from zhigou_toolchain.lifecycle.security import storage_key
