...
import {DeliveryChecklist,EvolutionBatchDownload} from './handoff-download';
...
export function FiveStageModeling(){
  ...
  const agentId=str(object(object(state.modeling_flow).agent_roles)[String(stage)]);
  const agentRecords=selected.flatMap(r=>rows(object(r.result.agent_execution).records)).filter(r=>r.stage_id===stage);
  ...
  <DeliveryChecklist/>
  {stage===5&&<EvolutionBatchDownload/>}
  ...
  <details><summary>角色 工具与版本交接</summary>{agentRecords.length?<ArtifactViewer value={{agent_id:agentId,records:agentRecords}}/>:<p>NOT_RUN：没有本轮角色执行收据。</p>}</details>
