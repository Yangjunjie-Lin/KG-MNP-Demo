...
export function DataTable({data, fields}: {data: Document[]; fields: [string, string][]}) {
  ...
  return <><label className="search" htmlFor={id}>搜索表格<input id={id} value={search} onChange={e => {setSearch(e.target.value); table.setPageIndex(0);}} /></label><div className="table-scroll"><table><thead>{table.getHeaderGroups().map(group => <tr key={group.id}>{group.headers.map(h => <th key={h.id}>{flexRender(h.column.columnDef.header, h.getContext())}</th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.map(row => <tr key={row.id}>{row.getVisibleCells().map(cell => <td key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}</tbody></table></div>{!filtered.length && <p className="empty">没有匹配记录</p>}<div className="pager"><button disabled={!table.getCanPreviousPage()} onClick={() => table.previousPage()}>上一页</button><span>{table.getState().pagination.pageIndex + 1} / {Math.max(1, table.getPageCount())} · {filtered.length} 条</span><button disabled={!table.getCanNextPage()} onClick={() => table.nextPage()}>下一页</button></div></>;
}
