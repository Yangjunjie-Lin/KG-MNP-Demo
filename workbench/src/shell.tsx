...
export function App() {
  ...
  return <Routes><Route path="/" element={<Projects principal={session.data.principal} />} /><Route path="/projects/:projectId" element={<Workspace principal={session.data.principal} />}><Route index element={<Navigate to="overview" replace />} /><Route path="overview" element={<Overview />} /><Route path="sources" element={<Sources />} /><Route path="modeling" element={<Suspense fallback={<p role="status">正在加载建模视图…</p>}><Modeling /></Suspense>} /><Route path="releases" element={<Releases />} /><Route path="versions" element={<Versions />} /><Route path="integrations" element={<Integrations />} /><Route path="jobs" element={<Jobs />} /></Route><Route path="*" element={<main className="login"><h1>页面不存在</h1><Link to="/">返回项目选择</Link></main>} /></Routes>;
}
